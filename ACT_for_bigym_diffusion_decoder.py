...
from env.make_bigym_envs import test_in_simulation_diffusion_style
...
if __name__ == "__main__":
    ...
    if configs["method"] == "DDPM":
        ...
        assert configs["method"] == "DDIM", "只有 DDPM 和 DDIM 两种方法存在!"
        from network.ACT_DDIM_decoder_bigym import ActionChunkingTransformer
    ...
    for global_step, batch in tqdm(enumerate(dataloader)):
        ...
        # 具有扩散头的解码器, 输出的是:
        # (预测噪声, 原始噪声), (z分布均值, z分布方差)
        (predicted_noise, noise), (mu, logvar) = model(input_obs, input_action)

        # 计算损失
        # 计算不同级别的 kl 散度
        total_kld, dim_wise_kld, mean_kld = kl_divergence(mu, logvar)
        # 扩散模型的训练目标是预测噪声, 为了代码的整体性, 这里还是用 l1 变量名
        all_l1 = F.mse_loss(predicted_noise, noise, reduction='none')
        l1 = all_l1.mean()
        loss_dict = dict()
        loss_dict['l1'] = l1
        ...
        if global_step % configs["eval_frequency"] == 0:
            ...
            # 具有扩散头的解码器, 输出的是:
            # (预测噪声, 原始噪声), (z分布均值, z分布方差)
            (predicted_noise, noise), (mu, logvar) = model.forward(input_obs, input_action)

            if configs["method"] == "DDIM":
                # 改成 DDIM , 我的推理时候去噪步数为原始步数的 10 分之一
                predicted_action = model.inference(input_obs, model.num_diffusion_steps // 10)
                # 在验证集再加一个预测动作和原始动作的 l1 损失
                action_loss = F.l1_loss(predicted_action, input_action)
                writer.add_scalar(f"validation/action_loss", action_loss, global_step)
        ...
        if (global_step > 0) and (global_step % configs["test_frequency"]) == 0:
            # 进入测试模式
            test_result = test_in_simulation_diffusion_style(
                model=model,
                env=env,
                config=configs,
                device=device,

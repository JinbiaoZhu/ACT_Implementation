...
from network.components.tools import freeze_except_decoder, count_model_params
...
if __name__ == "__main__":
    ...
    run_name = f"{configs['exp_name']}-" + \
               f"{configs['envs']['env_id']}-decoder-finetune-" + \
               f"{configs['seed']}-" + \
    ...
    model.load_state_dict(
        torch.load(configs["origin_ckpt_path"])["agent"]
    )
    model = model.to(dtype=dtype, device=device)  # 可能用 torch.bfloat16 这样的数据类型
    # ==========> 冻结模型的编码器梯度
    freeze_except_decoder(model)  # 作为对比实验, 不进行冻结梯度, 就把这个函数注释掉, 也就是全部梯度计算
    # ==========> 计算并打印可训练参数占比
    total, trainable, ratio = count_model_params(model)
    print(f"总参数: {total}; 可训练参数: {trainable}; 比率: {ratio}")
    # ==========> 优化器设置
    # 只将参数中 requires_grad 为 True 的参数传给优化器
    trainable_params = filter(lambda p: p.requires_grad, model.parameters())
    optimizer = optim.AdamW(trainable_params, lr=configs["lr"], weight_decay=configs["weight_decay"])
    ...
    for global_step, batch in tqdm(enumerate(dataloader)):
        ...
        loss_dict['loss'] = l1  # 微调部分不涉及 kl 散度下降 + total_kld[0] * configs["kl_weight"]

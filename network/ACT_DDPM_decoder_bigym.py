...
class ActionChunkingTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # 扩散过程相关参数设置
        # --------------------------
        self.num_diffusion_steps = 100  # 扩散步数, 可根据需要调整
        beta_start = 0.0001
        beta_end = 0.02
    ...
    def inference(self, obs, num_steps=None):
        ...
        2. 从纯噪声 x 开始，迭代反向扩散更新，最终输出去噪后的动作预测。
        ...
        # (height*width*num_views, bs, d_model)
        pos_embed_cam = pos_embed_cam.flatten(2).permute(2, 0, 1).repeat(1, bs, 1)
        additional_pos_embed = self.additional_pos_embed.weight.unsqueeze(1).repeat(1, bs, 1)  # (2, bs, d_model)
        pos_embed_total = torch.cat([additional_pos_embed, pos_embed_cam],
                                    dim=0)  # (2+height*width*num_views, bs, d_model)
        ...
        for t in reversed(range(num_steps)):
            ...
            alpha_t = self.alphas[t_tensor].view(bs, 1, 1)  # (bs,1,1)
            beta_t = self.betas[t_tensor].view(bs, 1, 1)  # (bs,1,1)
            ...
            x = (1 / torch.sqrt(alpha_t)) * (x - (beta_t / sqrt_one_minus_alpha_bar) * predicted_noise) + torch.sqrt(
                beta_t) * z
